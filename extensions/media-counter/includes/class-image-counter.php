 ...
 * @since 1.1.0
 */
class KSM_Extensions_MediaCounter_ImageCounter {
	 ...
	 * @since 1.1.0
	 */
	const OPTION_NEEDS_INIT = 'ksm_extensions_media_counter_image_needs_init';
	 ...
	 * @since 1.1.0
	 */
	const OPTION_INITIALIZED = 'ksm_extensions_media_counter_image_initialized';
	 ...
	 * @since 1.1.0
	 */
	const CRON_HOOK = 'ksm_extensions_media_counter_image_bulk_init';
	 ...
	 * @since 1.1.0
	 */
	const AJAX_ACTION_DISMISS_NOTICE = 'ksm_extensions_dismiss_image_counter_notice';
	 ...
	 * @since 1.1.0
	 */
	const CACHE_GROUP = 'ksm_extensions_media_counter_image';
	...
	public function __construct( $core ) {
		$this->core     = $core;
		$this->meta_key = KSM_Extensions_MediaCounter_Core::META_KEYS['image'];
		$this->register_hooks();
	}
	...
	private function register_hooks() {
		...
		// Only register admin columns if images are enabled and admin columns are enabled
		if ( $this->core->get_option( 'count_images', true ) && $this->core->get_option( 'show_in_admin_columns', true ) ) {
		...
		wp_add_inline_script(
			...
			sprintf(
				...
				jQuery( document ).ready( function( $ ) {
					$( document ).on( "click", ".ksm-image-counter-notice .notice-dismiss", function() {
						$.post( ajaxurl, {
							action: "%1$s",
							nonce: "%2$s"
						} );
	...
	public function add_image_count_column( $columns ) {
		// Check if images are enabled and admin columns are enabled
		if ( ! $this->core->get_option( 'count_images', true ) || ! $this->core->get_option( 'show_in_admin_columns', true ) ) {
			return $columns;
		}
		
		$columns['ksm_extensions_media_counter_image_count'] = __( 'Images', 'ksm-extensions' );
		return $columns;
	}
	...
	public function display_image_count_column( $column, $post_id ) {
		if ( 'ksm_extensions_media_counter_image_count' !== $column ) {
			return;
		}
		...
		if ( $count > 0 ) {
			printf( '<span class="ksm-image-count">%s</span>', esc_html( number_format_i18n( $count ) ) );
		} else {
			echo '<em>—</em>';
		}
	}
	...
	public function make_image_count_sortable( $columns ) {
		$columns['ksm_extensions_media_counter_image_count'] = 'ksm_extensions_media_counter_image_count';
		return $columns;
	}
	...
	public function handle_image_count_sorting( $query ) {
		...
		if ( 'ksm_extensions_media_counter_image_count' === $query->get( 'orderby' ) ) {
	...
	public function admin_notices() {
		...
		echo '<div class="notice notice-info is-dismissible ksm-image-counter-notice">';
		echo '<p>' . esc_html__( 'KSM Extensions is initializing existing image counts. This will run in the background and may take a few moments.', 'ksm-extensions' ) . '</p>';
		echo '</div>';
	}
	...
	public function get_image_count( $post_id ) {
		...
		$cache_key = 'ksm_extensions_media_counter_image_count_' . $post_id;
	...
	public function cleanup_image_count_meta( $post_id ) {
		...
		wp_cache_delete( 'ksm_extensions_media_counter_image_count_' . $post_id, self::CACHE_GROUP );
