 ...
 *   Reimplemented and modernised for KSM Extensions with permission under GPLv2+.
 *
 ...
 * @since 1.6.0
 */
class KSM_Extensions_AutoUploadImages_Handler {
	...
	private function download_remote_image() {
		...
		if ( 200 !== $code ) {
			return new WP_Error( 'ksm_extensions_auto_upload_http_error', esc_html__( 'Remote server returned an error.', 'ksm-extensions' ) );
		}

		$body = wp_remote_retrieve_body( $response );
		if ( empty( $body ) ) {
			return new WP_Error( 'ksm_extensions_auto_upload_empty', esc_html__( 'Remote response was empty.', 'ksm-extensions' ) );
		}
		...
		$tmp = wp_tempnam();
		if ( ! $tmp ) {
			return new WP_Error( 'ksm_extensions_auto_upload_tmp', esc_html__( 'Unable to create temporary file.', 'ksm-extensions' ) );
		}
		...
		if ( ! $mime || 0 !== strpos( $mime, 'image/' ) ) {
			@unlink( $tmp );
			return new WP_Error( 'ksm_extensions_auto_upload_mime', esc_html__( 'Downloaded file is not a valid image.', 'ksm-extensions' ) );
		}
	...
	private function store_file( array $remote ) {
		...
		if ( ! empty( $uploads['error'] ) ) {
			return new WP_Error( 'ksm_extensions_auto_upload_dir', $uploads['error'] );
		}
		...
		if ( ! $already_existing ) {
			...
			if ( ! file_exists( $path ) ) {
				@unlink( $remote['tmp'] );
				return new WP_Error( 'ksm_extensions_auto_upload_write', esc_html__( 'Failed to save the downloaded image.', 'ksm-extensions' ) );
			}
		}
